const RU = {
  location: "Москва",
  check_address: "Проверить адрес",
  delivertime: "Среднее время доставки*:",
  brand_name: "Куда Пицца",
  worktimedata: "Время работы:",
  worktime: " с 11:00 до 23:00",
  account: "Войти в аккаунт",
  check_address_title: "Проверить адрес доставки",
  check: "Проверить",
  address_placeholder:"Адрес" ,
  deliver_info: "Доставка пиццы в Москве",
  deliver_head_title: "Захотелось чего-то вкусного и сытного? Желание простое и понятное, только в холодильнике все не то, и до магазина идти лень. Все пропало? Нет. Недорого заказать пиццу в Москве очень просто! Вам на помощь спешит супергерой – Domino’s Pizza! Как у всякого супергероя, у Domino’s Pizza есть свои суперсилы: восхитительный вкус продукции из отборных ингредиентов; широкий ассортимент, включающий легендарные, фирменные и классические виды, для вегетарианцев и любителей экспериментировать; быстрая и бесплатная доставка пиццы в течение 30 минут, чтобы вкусное и ароматное блюдо не успевало остыть.",
  order: "Как сделать заказ",
  deliver_last_title: "Доставка пиццы от Domino’s – это когда Вам не нужно никуда ехать или звонить, ведь есть Интернет. Никогда еще заказ пиццы на дом в Москве не был таким простым! Чтобы заказать пиццу онлайн, Вам необходимо: выбрать понравившийся вариант и количество порций; положить желаемое в «Корзину»; не уходить далеко, так как вкусная пицца на заказ с доставкой уже мчится к Вам из ближайшей пиццерии Domino’s. И не забудьте оплатить заказ курьеру!",
  more: "Показать полностью",
  about: "О компании",
  rules: "Пользовательское соглашение",
  warrant: "Kafolat shartlari",
  help: "Условия гарантии",
  restauran: "Ресторан",
  contact: "Контакты",
  support: "Поддержка",
  track_order: "Отследить заказ",
  choose: "Выбрать",
  discount: "Акции",
  yourorder:"Ваш заказ" ,
  filter:"Фильтр" ,


  categories: [
    { name: "Пицца", image: "/pizza.svg" },
    { name: "Суши", image: "/sushi.svg" },
    { name: "Напитки", image: "/drink.svg" },
    { name: "Закуски", image: "/snack.svg" },
    { name: "Десерты", image: "/desert.svg" },
    { name: "Соусы", image: "/sauce.svg" },
    { name: "Комбо", image: "/combo.svg" },
  ],
  products: [
    {
      id: "pizza-1",
      category: "Пицца",
      filter: "New",
      image: "/pizza-img-1.png",
      name: "Чикен Сладкий Чили",
      description:
        "Курица, Лук, Перец Халапеньо, Сыр Моцарелла, Томатный соу...",
      price: 389,
    },
    {
      id: "pizza-2",
      category: "Пицца",
      filter: "Хит",
      image: "/pizza-img-2.png",
      name: "EASY PEASY огуречный расколбас",
      description:
        "Курица, Лук, Перец Халапеньо, Лук, Перец Халапеньо , Лук,..",
      price: 549,
    },
    {
      id: "pizza-3",
      category: "Пицца",
      filter: "",
      image: "/pizza-img-3.png",
      name: "EASY PEASY чикен а-ля хрен",
      description: "Курица, Лук, Соус Карбонара,Лук, Соус Карбонара.....",
      price: 249,
    },
    {
      id: "pizza-4",
      category: "Пицца",
      filter: "",
      image: "/pizza-img-4.png",
      name: "4 сезона",
      description:
        "Бекон, Ветчина, Грибы, Курица, Лук, Маслины, Огурцы мари...",
      price: 630,
    },
    {
      id: "pizza-5",
      category: "Пицца",
      filter: "",
      image: "/pizza-img-5.png",
      name: "4 сезона",
      description:
        "Бекон, Ветчина, Грибы, Курица, Лук, Маслины, Огурцы мари...",
      price: 630,
    },
    {
      id: "pizza-6",
      category: "Пицца",
      filter: "Хит",
      image: "/pizza-img-6.png",
      name: "EASY PEASY чикен а-ля хрен",
      description:
        "Курица, Лук, Соус Карбонара,Курица, Лук, Соус Карбонара,Лук, ...",
      price: 249,
    },
    {
      id: "pizza-7",
      category: "Пицца",
      filter: "",
      image: "/pizza-img-7.png",
      name: "Чикен Сладкий Чили",
      description:
        "Курица, Лук, Перец Халапеньо, Сыр Моцарелла, Томатный соу...",
      price: 399,
    },
    {
      id: "pizza-8",
      category: "Пицца",
      filter: "",
      image: "/pizza-img-8.png",
      name: "EASY PEASY огуречный расколбас",
      description:
        "Курица, Лук, Перец Халапеньо , Курица, Лук, Перец Халапеньо...",
      price: 549,
    },
    // {
    //   id: "pizza-9",
    //   category: "Пицца",
    //   filter: "",
    //   image: "/pizza-img-9.png",
    //   name: "EASY PEASY огуречный расколбас",
    //   description: "Курица, Лук, Перец Халапеньо...",
    //   price: 725
    // },
    // {
    //   id: "pizza-10",
    //   category: "Пицца",
    //   filter: "",
    //   image: "/pizza-img-10.png",
    //   name: "EASY PEASY огуречный расколбас",
    //   description: "Курица, Лук, Перец Халапеньо...",
    //   price: 349
    // },
    // {
    //   id: "pizza-11",
    //   category: "Пицца",
    //   filter: "",
    //   image: "/pizza-img-11.png",
    //   name: "EASY PEASY огуречный расколбас",
    //   description: "Курица, Лук, Перец Халапеньо...",
    //   price: 725
    // },
    // {
    //   id: "pizza-12",
    //   category: "Пицца",
    //   filter: "",
    //   image: "/pizza-img-12.png",
    //   name: "EASY PEASY огуречный расколбас",
    //   description: "Курица, Лук, Перец Халапеньо...",
    //   price: 349
    // },

    {
      id: "sushi-1",
      category: "Суши",
      filter: "New",
      image: "/sushi-img-1.png",
      name: "Филадельфия кранч",
      description:
        "Семга, рис, сыр креметто, соус унаги, креветка, авокадо, чип...",
      price: 475,
    },
    {
      id: "sushi-2",
      category: "Суши",
      filter: "Xit",
      image: "/sushi-img-2.png",
      name: "Филадельфия крем-брюле",
      description:
        "Сливочный сыр, семга татаки с тростниковым сахаром, соус у...",
      price: 395,
    },
    {
      id: "sushi-3",
      category: "Суши",
      filter: "",
      image: "/sushi-img-3.png",
      name: "Супер Филадельфия",
      description:
        "Действительно много семги, сливочный сыр, огурец, рис, н...",
      price: 425,
    },
    {
      id: "sushi-4",
      category: "Суши",
      filter: "",
      image: "/sushi-img-4.png",
      name: "Тигр мама",
      description: "Тигровая креветка, огурец, авокадо, соус Айоли, рис, ик...",
      price: 525,
    },
    {
      id: "sushi-5",
      category: "Суши",
      filter: "",
      image: "/sushi-img-5.png",
      name: "Тигр мама",
      description: "Тигровая креветка, огурец, авокадо, соус Айоли, рис, ик...",
      price: 525,
    },
    {
      id: "sushi-6",
      category: "Суши",
      filter: "",
      image: "/sushi-img-6.png",
      name: "Супер Филадельфия",
      description:
        "Действительно много семги, сливочный сыр, огурец, рис, н...",
      price: 425,
    },
    {
      id: "sushi-7",
      category: "Суши",
      filter: "",
      image: "/sushi-img-7.png",
      name: "Филадельфия кранч",
      description:
        "Семга, рис, сыр креметто, соус унаги, креветка, авокадо, чип...",
      price: 475,
    },
    {
      id: "sushi-8",
      category: "Суши",
      filter: "",
      image: "/sushi-img-8.png",
      name: "Филадельфия крем-брюле",
      description:
        "Сливочный сыр, семга татаки с тростниковым сахаром, соус у...",
      price: 395,
    },
    {
      id: "drink-1",
      category: "Напитки",
      filter: "New",
      image: "/drink-img-1.png",
      name: "Филадельфия кранч",
      description:
        "Семга, рис, сыр креметто, соус унаги, креветка, авокадо, чип...",
      price: 475,
    },
    {
      id: "drink-2",
      category: "Напитки",
      filter: "Хит",
      image: "/drink-img-2.png",
      name: "Филадельфия крем-брюле",
      description:
        "Сливочный сыр, семга татаки с тростниковым сахаром, соус у...",
      price: 395,
    },
    {
      id: "drink-3",
      category: "Напитки",
      filter: "",
      image: "/drink-img-3.png",
      name: "Супер Филадельфия",
      description:
        "Действительно много семги, сливочный сыр, огурец, рис, н...",
      price: 425,
    },
    {
      id: "drink-4",
      category: "Напитки",
      filter: "",
      image: "/drink-img-4.png",
      name: "Тигр мама",
      description: "Тигровая креветка, огурец, авокадо, соус Айоли, рис, ик...",
      price: 425,
    },
    {
      id: "drink-5",
      category: "Напитки",
      filter: "",
      image: "/drink-img-5.png",
      name: "Супер Филадельфия",
      description: "",
      price: 425,
    },
    {
      id: "drink-6",
      category: "Напитки",
      filter: "",
      image: "/drink-img-6.png",
      name: "Тигр мама",
      description: "",
      price: 525,
    },
    {
      id: "drink-7",
      category: "Напитки",
      filter: "",
      image: "/drink-img-7.png",
      name: "Супер Филадельфия",
      description: "",
      price: 425,
    },
    {
      id: "drink-8",
      category: "Напитки",
      filter: "",
      image: "/drink-img-8.png",
      name: "Тигр мама",
      description: "",
      price: 525,
    },
    {
      id: "sauce-1",
      category: "Закуски",
      filter: "New",
      image: "/snack-img-1.png",
      name: "Филадельфия кранч",
      description:
        "Семга, рис, сыр креметто, соус унаги, креветка, авокадо, чип...",
      price: 475,
    },
    {
      id: "sauce-2",
      category: "Закуски",
      filter: "Xit",
      image: "/snack-img-2.png",
      name: "Филадельфия крем-брюле",
      description:
        "Сливочный сыр, семга татаки с тростниковым сахаром, соус у...",
      price: 395,
    },
    {
      id: "sauce-3",
      category: "Закуски",
      filter: "",
      image: "/snack-img-3.png",
      name: "Супер Филадельфия",
      description:
        "Действительно много семги, сливочный сыр, огурец, рис, н...",
      price: 425,
    },
    {
      id: "sauce-4",
      category: "Закуски",
      filter: "",
      image: "/snack-img-4.png",
      name: "Тигр мама",
      description: "Тигровая креветка, огурец, авокадо, соус Айоли, рис, ик...",
      price: 525,
    },
    {
      id: "sauce-5",
      category: "Закуски",
      filter: "",
      image: "/snack-img-5.png",
      name: "Тигр мама",
      description: "Тигровая креветка, огурец, авокадо, соус Айоли, рис, ик...",
      price: 525,
    },
    {
      id: "sauce-6",
      category: "Закуски",
      filter: "",
      image: "/snack-img-6.png",
      name: "Супер Филадельфия",
      description:
        "Действительно много семги, сливочный сыр, огурец, рис, н...",
      price: 425,
    },
    {
      id: "sauce-7",
      category: "Закуски",
      filter: "",
      image: "/snack-img-7.png",
      name: "Филадельфия крем-брюле",
      description:
        "Сливочный сыр, семга татаки с тростниковым сахаром, соус у...",
      price: 395,
    },
    {
      id: "sauce-8",
      category: "Закуски",
      filter: "",
      image: "/snack-img-8.png",
      name: "Филадельфия кранч",
      description:
        "Семга, рис, сыр креметто, соус унаги, креветка, авокадо, чип...",
      price: 475,
    },

    {
      id: "desert-1",
      category: "Десерты",
      image: "/desert-img-1.png",
      name: "Филадельфия кранч",
      description:
        "Семга, рис, сыр креметто, соус унаги, креветка, авокадо, чип...",
      filter: "NEW",
      price: 475,
    },
    {
      id: "desert-2",
      category: "Десерты",
      image: "/desert-img-2.png",
      name: "Филадельфия крем-брюле",
      description:
        "Сливочный сыр, семга татаки с тростниковым сахаром, соус у...",
      filter: "ХИТ",
      price: 395,
    },
    {
      id: "desert-3",
      category: "Десерты",
      image: "/desert-img-3.png",
      name: "Супер Филадельфия",
      description:
        "Действительно много семги, сливочный сыр, огурец, рис, н...",
      filter: "",
      price: 425,
    },
    {
      id: "desert-4",
      category: "Десерты",
      image: "/desert-img-4.png",
      name: "Тигр мама",
      description: "Тигровая креветка, огурец, авокадо, соус Айоли, рис, ик...",
      filter: "",
      price: 525,
    },
    {
      id: "desert-5",
      category: "Десерты",
      image: "/desert-img-5.png",
      name: "Тигр мама",
      description: "Тигровая креветка, огурец, авокадо, соус Айоли, рис, ик...",
      filter: "",
      price: 525,
    },
    {
      id: "desert-6",
      category: "Десерты",
      image: "/desert-img-6.png",
      name: "Филадельфия крем-брюле",
      description:
        "Сливочный сыр, семга татаки с тростниковым сахаром, соус у...",
      filter: "",
      price: 395,
    },
    {
      id: "desert-7",
      category: "Десерты",
      image: "/desert-img-7.png",
      name: "Филадельфия кранч",
      description:
        "Семга, рис, сыр креметто, соус унаги, креветка, авокадо, чип...",
      filter: "",
      price: 475,
    },
    {
      id: "desert-8",
      category: "Десерты",
      image: "/desert-img-8.png",
      name: "Супер Филадельфия",
      description:
        "Действительно много семги, сливочный сыр, огурец, рис, н...",
      filter: "",
      price: 425,
    },
    {
      id: "sauce-1",
      category: "Соусы",
      filter: "NEW",
      image: "/sauce-img-8.png",
      name: "Филадельфия кранч",
      description: "",
      price: 475,
    },
    {
      id: "sauce-2",
      category: "Соусы",
      filter: "ХИТ",
      image: "/sauce-img-2.png",
      name: "Филадельфия крем-брюле",
      description: "",
      price: 395,
    },
    {
      id: "sauce-3",
      category: "Соусы",
      filter: "",
      image: "/sauce-img-3.png",
      name: "Супер Филадельфия",
      description: "",
      price: 425,
    },
    {
      id: "sauce-4",
      category: "Соусы",
      filter: "",
      image: "/sauce-img-4.png",
      name: "Тигр мама",
      description: "",
      price: 525,
    },
    {
      id: "sauce-5",
      category: "Соусы",
      filter: "",
      image: "/sauce-img-5.png",
      name: "Филадельфия крем-брюле",
      description: "",
      price: 395,
    },
    {
      id: "sauce-6",
      category: "Соусы",
      filter: "",
      image: "/sauce-img-4.png",
      name: "Тигр мама",
      description: "",
      price: 525,
    },
    {
      id: "sauce-7",
      category: "Соусы",
      filter: "",
      image: "/sauce-img-7.png",
      name: "Супер Филадельфия",
      description: "",
      price: 425,
    },
    {
      id: "sauce-8",
      category: "Соусы",
      filter: "",
      image: "/sauce-img-8.png",
      name: "Филадельфия кранч",
      description: "",
      price: 475,
    },
    {
      id: "combo-1",
      category: "Комбо",
      filter: "",
      image: "/combo-img-1.png",
      name: "Филадельфия кранч",
      description:
        "Семга, рис, сыр, креметто, соус унаги, креветка, авокадо, чип...",
      price: 475,
      discount: 22,
    },
    {
      id: "combo-2",
      category: "Комбо",
      filter: "",
      image: "/combo-img-2.png",
      name: "Филадельфия крем-брюле",
      description:
        "Сливочный сыр, семга татаки с тростниковым сахаром, соус у...",
      price: 395,
      discount: 18,
    },
    {
      id: "combo-3",
      category: "Комбо",
      filter: "",
      image: "/combo-img-3.png",
      name: "Супер Филадельфия",
      description:
        "Действительно много семги, сливочный сыр, огурец, рис, н...",
      price: 425,
      discount: 10,
    },
    {
      id: "combo-4",
      category: "Комбо",
      filter: "",
      image: "/combo-img-4.png",
      name: "Тигр мама",
      description: "Тигровая креветка, огурец, авокадо, соус Айоли, рис, ик...",
      price: 525,
      discount: 15,
    },
    {
      id: "combo-5",
      category: "Комбо",
      filter: "",
      image: "/combo-img-5.png",
      name: "Филадельфия крем-брюле",
      description:
        "Сливочный сыр, семга татаки с тростниковым сахаром, соус у...",
      price: 395,
      discount: 18,
    },
    {
      id: "combo-6",
      category: "Комбо",
      filter: "",
      image: "/combo-img-6.png",
      name: "Филадельфия кранч",
      description:
        "Семга, рис, сыр креметто, соус унаги, креветка, авокадо, чип...",
      price: 475,
      discount: 22,
    },
    {
      id: "combo-7",
      category: "Комбо",
      filter: "",
      image: "/combo-img-7.png",
      name: "Тигр мама",
      description: "Тигровая креветка, огурец, авокадо, соус Айоли, рис, ик...",
      price: 525,
      discount: 15,
    },
    {
      id: "combo-8",
      category: "Комбо",
      filter: "",
      image: "/combo-img-8.png",
      name: "Супер Филадельфия",
      description:
        "Действительно много семги, сливочный сыр, огурец, рис, н...",
      price: 425,
      discount: 10,
    },
  ],
};

export default RU;
